...
impl BackendRegistry {
    ...
    ///
    /// If `gpu_variant` is `Some`, only removes that variant.
    /// If `None`, removes all variants.
    pub fn remove(&mut self, name: &str, gpu_variant: Option<&str>) -> Result<()> {
        delete_all_backend_versions(&self.conn, name, gpu_variant)
    ...
    pub fn get(&self, name: &str, gpu_variant: &str) -> Result<Option<BackendInfo>> {
        let record = get_active_backend(&self.conn, name, gpu_variant)
    ...
    ///
    /// If `gpu_variant` is `Some`, only returns that variant.
    ...
    pub fn list_all_versions(
        ...
        let records = list_backend_versions(&self.conn, name, gpu_variant)
    ...
    /// Activate a specific version of a backend for a given gpu_variant.
    ///
    /// Deactivates all other versions of the same variant and activates the requested one.
    /// Returns Ok(true) if the version was found and activated, Ok(false) if not found.
    pub fn activate(&mut self, name: &str, gpu_variant: &str, version: &str) -> Result<bool> {
        activate_backend_version(&self.conn, name, gpu_variant, version).with_context(|| {
            format!(
                "Failed to activate backend '{}' variant '{}' version '{}'",
                name, gpu_variant, version
            )
        })
    }

    /// Remove a single (name, gpu_variant, version) installation from the registry.
    ///
    ...
    /// (no active version remains for that backend name+variant).
    pub fn remove_version(&mut self, name: &str, gpu_variant: &str, version: &str) -> Result<()> {
        // Get the record before deleting, to check if it was active and to get the path
        let record =
            get_backend_by_version(&self.conn, name, gpu_variant, version).with_context(|| {
                format!(
                    "Failed to query backend '{}' variant '{}' version '{}'",
                    name, gpu_variant, version
                )
            })?;
        ...
        // Delete the DB row
        delete_backend_installation(&self.conn, name, gpu_variant, version).with_context(|| {
            format!(
                "Failed to remove backend '{}' variant '{}' version '{}'",
                name, gpu_variant, version
            )
        })?;
        ...
        if was_active {
            let remaining = list_backend_versions(&self.conn, name, Some(gpu_variant))
                .with_context(|| {
            ...
            if !remaining.is_empty() {
                ...
                activate_backend_version(&self.conn, name, gpu_variant, &newest.version)
                    .with_context(|| {
                        format!(
...
impl BackendRegistry {
    ...
    fn backend_info_to_record(backend: &BackendInfo) -> Result<BackendInstallationRecord> {
        ...
        Ok(BackendInstallationRecord {
            ...
            gpu_variant: backend.gpu_variant.clone(),
            source: source_json,
            is_active: true,
        })
    }
...
mod tests {
    ...
    fn test_registry_remove() {
        ...
        registry.remove("llama_cpp", None).unwrap();
    ...
    fn test_registry_update_version() {
        ...
        let backend = registry.get("llama_cpp", "cpu").unwrap().unwrap();
    ...
    fn test_registry_get_returns_none_for_unknown() {
        ...
        let result = registry.get("nonexistent", "cpu").unwrap();
    ...
    fn test_registry_list_all_versions() {
        ...
        assert!(registry
            .list_all_versions("nonexistent", None)
            .unwrap()
            .is_none());
        ...
        let versions = registry
            .list_all_versions("llama_cpp", None)
            .unwrap()
            .unwrap();
    ...
    fn test_registry_activate_version() {
        ...
        let active = registry.get("llama_cpp", "cpu").unwrap().unwrap();
        assert_eq!(active.version, "b9000");

        // Activate b8407
        let result = registry.activate("llama_cpp", "cpu", "b8407").unwrap();
        assert!(result);
    ...
    fn test_registry_remove_last_version_deactivates_others() {
        ...
        registry
            .remove_version("llama_cpp", "cpu", "b9000")
            .unwrap();
    ...
    fn test_registry_remove_last_version_cleans_up() {
        ...
        let active = registry.get("llama_cpp", "cpu").unwrap();
